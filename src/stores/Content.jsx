...
export default class ContentStore {
  ...
  constructor(rootStore) {
    ...
    axios.get(settings.contentUrl).then(res => {
      this.content = res.data || null;
      this.contentLoaded = true;
      this.showNotification = !localStorage.getItem(
        `StabilityFeeChangeAlertClosed-${this.stabilityFeeMarkdown()}`
      );
      ...
      for (let key in this.content.faq) {
        if (key.substr(0, 13) === "notification-" && key.substr(-3) === "-cn")
          this.content.notifications[key] = {
            ...
            show: !localStorage.getItem(
              `NotificationClosed-${key}-${this.content.faq[key].markdown}`
            )
          };
      }
      ...
      switch (capture) {
        case "stability_fee":
          return (
            formatNumber(
              toWei(
                fromWei(this.rootStore.system.tub.fee).pow(60 * 60 * 24 * 365)
              )
                .times(100)
                .minus(toWei(100)),
              1
        ...
        case "liquidation_penalty":
          return (
            formatNumber(this.rootStore.system.tub.axe.minus(WAD).times(100)) +
            "%"
          );
  ...
  stabilityFeeMarkdown = () =>
    (this.getHelpItem("stability-fee-information-cn") || {}).markdown;
  stabilityFeeContent = () =>
    this.stabilityFeeMarkdown() && compiler(this.stabilityFeeMarkdown());
  ...
  hideGeneralNotification = key => {
    localStorage.setItem(
      `NotificationClosed-${key}-${this.content.notifications[key].markdown}`,
      true
    );
