...
export default class SystemStore {
  ...
  @computed get showLegacyAlert() {
    return !Object.keys(this.tub.legacyCups).every(
      elem => Object.keys(this.tub.cups).indexOf(elem) > -1
    );
  }
  ...
  init = (top, tub, tap, vox, pit, pip, pep, gem, gov, skr, dai, sin) => {
    if (
      ...
      !this.rootStore.network.stopIntervals
    ) {
      ...
      .then(r => {
        ...
        if (this.rootStore.transactions.setLatestBlock(block, firstLoad)) {
          ...
          for (const [index, val] of sValues.entries()) {
            this[val[0]][val[1]] =
              val[2] || false ? fromRaytoWad(r[6][index]) : r[6][index];
          }
          // Set token values
          for (const [index, val] of tValues.entries()) {
            ...
            if (
              val[1] === "allowance" &&
              this.rootStore.transactions.loading.hasOwnProperty(
                "changeAllowance"
              ) &&
              ...
              setTimeout(
                () =>
                  this.rootStore.transactions.cleanLoading(
                    "changeAllowance",
                    val[0]
                  ),
                100
          ...
          console.debug(
            `Ignoring returned values (latest block #${this.rootStore.transactions.latestBlock}, response block #${block}). Trying again...`
          );
          setTimeout(
  ...
  getCup = (id, firstLoad = false) => {
    return new Promise((resolve, reject) => {
      daisystem.getCup(id).then(
        cup => {
          console.debug("Got cup:", cup);
          if (
            this.rootStore.transactions.setLatestBlock(cup.block, firstLoad)
          ) {
            resolve(cup);
          } else {
            console.debug(
              `Error loading cup (latest block ${this.rootStore.transactions.latestBlock}, request one: ${cup.block}}, trying again...`
            );
            setTimeout(() => {
    ...
    return new Promise((resolve, reject) => {
      ...
      promisesLogs.push(
        new Promise((resolve, reject) => {
          blockchain.objects.tub
            .LogNewCup({ lad }, { fromBlock })
            .get((e, r) => {
              if (!e) {
                for (let i = 0; i < r.length; i++) {
                  promisesCups.push(
                    this.getCup(parseInt(r[i].args.cup, 16), firstLoad)
                  );
                }
                resolve(true);
              } else {
      ...
      promisesLogs.push(
        new Promise((resolve, reject) => {
          blockchain.objects.tub
            .LogNote(
              { sig: methodSig("give(bytes32,address)"), bar: toBytes32(lad) },
              { fromBlock }
            )
            .get((e, r) => {
              if (!e) {
                for (let i = 0; i < r.length; i++) {
                  promisesCups.push(
                    this.getCup(parseInt(r[i].args.foo, 16), firstLoad)
                  );
                }
                resolve(true);
              } else {
  ...
  setCups = async (
    ...
    try {
      const serviceData = settings.chain[this.rootStore.network.network].service
        ? await daisystem.getCupsFromService(
            this.rootStore.network.network,
            lad
          )
        : [];
      ...
      promisesCups = await this.getCupsFromChain(
        lad,
        fromBlock,
        promisesCups,
        firstLoad
          ...
          if (type === "new") {
            if (this.tub.cupsLoading) {
              ...
              if (this.tub.cupsLoading) {
                ...
                setTimeout(
                  () => this.setMyCupsFromChain(true, callbacks),
                  3000
                );
              } else if (
                !this.rootStore.network.stopIntervals &&
                keys.length > 0 &&
                settings.chain[this.rootStore.network.network].service
              ) {
                keys.forEach(key => {
  ...
  loadCupHistory = id => {
    ...
    if (settings.chain[this.rootStore.network.network].service) {
      Promise.resolve(
        daisystem.getCupHistoryFromService(this.rootStore.network.network, id)
      ).then(
        history => {
          if (history) {
            let cup = { ...this.tub.cups[id] };
            cup.history = history;
            this.tub.cups[id] = cup;
            const notification = daisystem.getBiteNotification(
              id,
              history,
              localStorage.getItem(`CDPLiquidated${history[0].time}Closed`)
            );
            if (notification) {
              this.rootStore.transactions.notificator.notice(
                Math.random(),
                "CDP Liquidated",
                notification,
                0,
                () =>
                  localStorage.setItem(
                    `CDPLiquidated${history[0].time}Closed`,
                    true
                  )
              );
            }
  ...
  calculateLiquidationPrice = (skr, dai) => {
    return daisystem.calculateLiquidationPrice(
      this.vox.par,
      this.tub.per,
      this.tub.mat,
      skr,
      ...
      if (!e) {
        if (this.filtersReceived.hasOwnProperty(r.transactionHash) !== true) {
          ...
          if (
            ...
            typeof r.args.foo !== "undefined" &&
            typeof this.tub.cups[parseInt(r.args.foo, 16)] !== "undefined"
          ) {
  ...
  setFiltersToken = token => {
    ...
    for (let i = 0; i < filters.length; i++) {
      ...
      if (blockchain.objects[token][filters[i]]) {
        blockchain.objects[token][filters[i]](
          conditions,
          { fromBlock: "latest" },
          (e, r) => {
            if (!e) {
              if (
                this.filtersReceived.hasOwnProperty(r.transactionHash) !== true
              ) {
                this.filtersReceived[r.transactionHash] = true;
                this.rootStore.transactions.logTransactionConfirmed(r);
                this.setAggregatedValues();
  ...
  changeAllowance = (token, value, callbacks = []) => {
    const title = `${value ? "Unlock" : "Lock"}: ${token
      .replace("gem", "weth")
      .replace("gov", "mkr")
      .replace("skr", "peth")
      .toUpperCase()}`;
    ...
    this.rootStore.transactions.askPriceAndSend(
      title,
      blockchain.objects[token].approve,
      [this.rootStore.profile.proxy, value ? -1 : 0],
      params,
      callbacks
    );
  };
  ...
  checkProxyAndSetAllowance = (token, value) => {
    ...
    this.rootStore.profile.checkProxy([
      [
        ...
        [
          [
            "system/setAggregatedValues",
            [["transactions/cleanLoadingOnError", "changeAllowance", token]]
          ]
        ]
      ]
    ]);
  ...
  transferToken = (token, to, amount) => {
    const title = `Transfer ${amount} ${token
      .replace("gov", "mkr")
      .toUpperCase()
      .replace("DAI", "SAI")} to ${truncateAddress(to)}`;
    if (token === "eth") {
      ...
      this.rootStore.transactions.askPriceAndSend(
        title,
        blockchain.objects[token].transfer,
        [to, toWei(amount)],
        params,
        [["system/setAggregatedValues"]]
      );
  ...
  migrateCDP = async (cup, callbacks) => {
    ...
    if (
      proxy &&
      isAddress(proxy) &&
      (await blockchain.getProxyOwner(proxy)) ===
        this.rootStore.network.defaultAccount
    ) {
  ...
  executeProxyTx = (action, value, gasLimit, notificator) => {
    ...
    this.rootStore.transactions.askPriceAndSend(
      ...
      [
        settings.chain[this.rootStore.network.network].saiProxyCreateAndExecute,
        action
      ],
      params,
      notificator.callbacks
    );
  };

  open = () => {
    const title = "Open CDP";
    const action = `${methodSig(`open(address)`)}${addressToBytes32(
      this.tub.address,
      false
    )}`;
    this.executeProxyTx(action, 0, this.shouldSetGasLimit() ? 120000 : null, {
      title,
      callbacks: [["system/setMyCupsFromChain"]]
    });
  };

  shut = (cupId, useOTC = false) => {
    const title = `Close CDP ${cupId}`;
    const action = `${methodSig(
      `shut(address,bytes32${useOTC ? ",address" : ""})`
    )}${addressToBytes32(this.tub.address, false)}${toBytes32(cupId, false)}${
      useOTC
        ? addressToBytes32(
            settings.chain[this.rootStore.network.network].otc,
            false
          )
        : ""
    }`;
    this.executeProxyTx(action, 0, this.shouldSetGasLimit() ? 1000000 : null, {
      title,
      callbacks: [
        ["system/setMyCupsFromChain"],
        ["system/setMyLegacyCupsFromChain"],
        ["system/setAggregatedValues"]
      ]
    });
  };
  ...
  give = (cupId, newOwner) => {
    ...
    )}${addressToBytes32(this.tub.address, false)}${toBytes32(
      cupId,
      false
    )}${addressToBytes32(newOwner, false)}`;
    this.executeProxyTx(action, 0, this.shouldSetGasLimit() ? 100000 : null, {
      title,
      callbacks: [["system/setMyCupsFromChain"]]
    });
  };
  ...
  lockAndDraw = (cupId, eth, dai) => {
    ...
    if (eth.gt(0) || dai.gt(0)) {
      if (!cupId) {
        callbacks = [
          ["system/setMyCupsFromChain", true],
          ["system/setAggregatedValues"]
        ];
        ...
        if (this.rootStore.profile.proxy) {
          ...
          this.rootStore.transactions.askPriceAndSend(
            title,
            blockchain.loadObject(
              "saiProxyCreateAndExecute",
              settings.chain[this.rootStore.network.network]
                .saiProxyCreateAndExecute
            ).createOpenLockAndDraw,
            [
              settings.chain[this.rootStore.network.network].proxyRegistry,
              this.tub.address,
              toWei(dai)
            ],
            params,
        ...
        callbacks = [
          ["system/reloadCupData", cupId],
          ["system/setAggregatedValues"]
        ];
        if (dai.equals(0)) {
          title = `Deposit ${eth.valueOf()} ETH`;
          action = `${methodSig(`lock(address,bytes32)`)}${addressToBytes32(
            this.tub.address,
            false
          )}${toBytes32(cupId, false)}`;
          gasLimit = 300000;
    ...
    if (eth.gt(0) || dai.gt(0)) {
      if (dai.equals(0)) {
        ...
        action = `${methodSig(
          ...
          cupId,
          false
        )}${toBytes32(toWei(eth), false)}`;
        gasLimit = 400000;
      } else if (eth.equals(0)) {
        title = `Payback ${dai.valueOf()} SAI`;
        action = `${methodSig(
          `wipe(address,bytes32,uint256${useOTC ? ",address" : ""})`
        )}${addressToBytes32(this.tub.address, false)}${toBytes32(
          cupId,
          false
        )}${toBytes32(toWei(dai), false)}${
        ...
        action = `${methodSig(
          ...
          cupId,
          false
        )}${toBytes32(toWei(eth), false)}${toBytes32(toWei(dai), false)}${
          useOTC
            ? addressToBytes32(
                settings.chain[this.rootStore.network.network].otc,
                false
  ...
  executeAction = async params => {
    ...
    switch (this.rootStore.dialog.method) {
      case "open":
        callbacks = [["system/open"]];
        break;
      case "lock":
        callbacks = [
          [
            "system/lockAndDraw",
            this.rootStore.dialog.cupId,
            value,
            toBigNumber(0)
          ]
        ];
      ...
      case "wipe":
        ...
        if (params.govFeeType === "mkr") {
          ...
          callbacks = [["system/checkAllowance", "gov", callbacks]];
        }
        break;
      case "free":
        callbacks = [
      ...
      case "shut":
        callbacks = [
          [
            "system/shut",
            this.rootStore.dialog.cupId,
            this.tub.cups[this.rootStore.dialog.cupId].art.gt(0) &&
              params.govFeeType === "dai"
          ]
        ];
        if (this.tub.cups[this.rootStore.dialog.cupId].art.gt(0)) {
          const futureGovDebtSai = this.futureRap(
            this.tub.cups[this.rootStore.dialog.cupId],
            1200
          );
          const futureGovDebtMKR = wdiv(futureGovDebtSai, this.pep.val).round(
            0
          );
          const valuePlusGovFee =
            params.govFeeType === "dai"
              ? this.tab(this.tub.cups[this.rootStore.dialog.cupId]).add(
                  futureGovDebtSai.times(1.25)
                )
              : this.tab(this.tub.cups[this.rootStore.dialog.cupId]); // If fee is paid in DAI we add an extra 25% (spread)
          if (valuePlusGovFee.gt(this.dai.myBalance)) {
            error = "没有足够的 Sai 关闭 CDP";
          } else if (
            params.govFeeType === "mkr" &&
            futureGovDebtMKR.gt(this.gov.myBalance)
          ) {
            error = "没有足够的 MKR 关闭 CDP";
          }
          callbacks = [["system/checkAllowance", "dai", callbacks]];
          if (params.govFeeType === "mkr") {
            // If fee will be paid with MKR it is necessary to check its allowance
            callbacks = [["system/checkAllowance", "gov", callbacks]];
          }
      ...
      case "give":
        if (params.giveHasProxy && params.giveToProxy) {
          ...
          if (
            proxy &&
            (await blockchain.getProxyOwner(proxy)) === value.toLowerCase()
          ) {
            value = proxy;
          } else {
            error = "无效的代理地址";
        ...
        break;
      case "migrate":
        this.rootStore.transactions.addLoading(
          "migrate",
          this.rootStore.dialog.cupId
        );
        callbacks = [
  ...
  shouldSetGasLimit() {
    return (
      this.rootStore.network.hw.active ||
      getWebClientProviderName() === "imtoken"
    );
  }
}
