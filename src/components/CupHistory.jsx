            ...
            <React.Fragment>
              <div
                className={
                  ...
                  (!this.showExpander ? " hide-expander" : "")
                }
              >
                {this.props.history &&
                  ...
                  this.props.history.map((action, key) => {
                    ...
                    switch (action.act) {
                      ...
                      case "LOCK":
                        message = (
                          <React.Fragment>
                            存入 {printNumber(toWei(action.arg * action.per))}{" "}
                            ETH ({printNumber(toWei(action.arg))} PETH) 到你的
                            CDP
                          </React.Fragment>
                        );
                      ...
                      case "FREE":
                        message = (
                          <React.Fragment>
                            取回 {printNumber(toWei(action.arg * action.per))}{" "}
                            ETH ({printNumber(toWei(action.arg))} PETH) 从你的
                            CDP
                          </React.Fragment>
                        );
                      ...
                      case "DRAW":
                        message = (
                          <React.Fragment>
                            生成 {printNumber(toWei(action.arg))} SAI 从你的 CDP
                          </React.Fragment>
                        );
                      ...
                      case "WIPE":
                        message = (
                          <React.Fragment>
                            偿还 {printNumber(toWei(action.arg))} SAI 到你的 CDP
                          </React.Fragment>
                        );
                      ...
                      case "SHUT":
                        message = (
                          <React.Fragment>关闭了你的 CDP</React.Fragment>
                        );
                        image = historyIconUnknown;
                        break;
                      case "BITE":
                        ...
                        const liqInk = toWei(
                          this.props.history[key + 1].ink - action.ink
                        );
                        ...
                        message = (
                          <React.Fragment>
                            ...
                            {printNumber(art, 2)} SAI. Total{" "}
                            {printNumber(liqETH)} ETH ({printNumber(liqInk)}{" "}
                            PETH) has been liquidated at {printNumber(pip, 2)}{" "}
                            USD.
                          </React.Fragment>
                        );
                      ...
                      <div className="history-cointainer" key={key}>
                        ...
                        <div className="history-details">
                          <div className="history-date">
                            {formatDate(new Date(action.time).getTime() / 1000)}
                          </div>
                          <span>{message}</span>
              ...
              >
                ...
                <div className="history-details">
                  <span onClick={() => this.toggleExpand()}>
                    {this.state.stepsExpanded ? "Collapse" : "Expand"}{" "}
                    完整历史记录
                  </span>
                </div>
              </div>
