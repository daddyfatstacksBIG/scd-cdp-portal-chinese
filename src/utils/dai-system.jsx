...
export const getCup = id => {
  ...
  return new Promise((resolve, reject) => {
    blockchain.objects.saiValuesAggregator.aggregateCDPValues(
      toBytes32(id),
      (e, cup) => {
        if (!e) {
          const cupData = {
...
export const getCupsFromService = (network, lad) => {
  return new Promise((resolve, reject) => {
    getFromService(
      network,
      `{ allCups( condition: { lad: "${toChecksumAddress(
        lad
      )}" } ) { nodes { id, block } } }`
...
export const getCupHistoryFromService = (network, cupId) => {
  return new Promise((resolve, reject) => {
    getFromService(
      network,
      `{ getCup(id: ${cupId}) { actions { nodes { act arg guy tx time ink art per pip } } } }`
    ).then(
      r => resolve(r.data.getCup ? r.data.getCup.actions.nodes : null),
      e => reject(e)
    );
  });
};
    ...
    return (
      <React.Fragment>
        <div className="grouped-section">
          Your CDP #{cupId} was liquidated on {date} to pay back{" "}
          {printNumber(art)} SAI.
        </div>
        <div className="grouped-section">
          <div className="dark-text">Total ETH (PETH) liquidated</div>
          <div style={{ fontSize: "1.3rem", fontWeight: "600" }}>
            {printNumber(liqETH)} ETH
          </div>
        ...
        <div className="grouped-section">
          <div className="dark-text">
            Became vulnerable to liquidation @ price
          </div>
          <div style={{ fontSize: "1.3rem", fontWeight: "600" }}>
            {printNumber(liqPrice)} USD
          </div>
...
export const futureRap = (cup, age, chi, rhi, tax, fee) => {
  return wmul(wmul(cup.ire, rhi), toWei(fromWei(wmul(tax, fee)).pow(age)))
    .minus(wmul(wmul(cup.art, chi), toWei(fromWei(tax).pow(age))))
    .round(0);
};

export const getContracts = (proxyRegistry, address) => {
  return new Promise((resolve, reject) => {
    blockchain.objects.saiValuesAggregator.getContractsAddrs(
      proxyRegistry,
      address,
      (e, r) => {
        if (!e) {
...
export const getAggregatedValues = (account, proxy) => {
  return new Promise((resolve, reject) => {
    blockchain.objects.saiValuesAggregator.aggregateValues.call(
      account,
      proxy,
